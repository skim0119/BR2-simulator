...
from br2.memory_block.memory_block_free_rod import (
...
def construct_memory_block_structures(systems):
    ...
    temp_list_for_free_rod_systems = []
    temp_list_for_rigid_body_systems = []
    temp_list_for_cosserat_rod_systems_idx = []
    temp_list_for_free_rod_systems_idx = []
    temp_list_for_rigid_body_systems_idx = []

    for system_idx, sys_to_be_added in enumerate(systems):
        if isinstance(sys_to_be_added, FreeCosseratRod):
            temp_list_for_free_rod_systems.append(sys_to_be_added)
            temp_list_for_free_rod_systems_idx.append(system_idx)
    ...
    if temp_list_for_free_rod_systems:
        _memory_blocks.append(
            MemoryBlockMuscularRod(
                temp_list_for_free_rod_systems,
                temp_list_for_free_rod_systems_idx,
