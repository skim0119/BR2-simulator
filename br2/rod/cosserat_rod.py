...
from elastica.rod.knot_theory import KnotTheory, _compute_twist
...
def _compute_geometry_from_state(position_collection, volume, lengths, tangents):
    ...
    for k in range(lengths.shape[0]):
        ...
        # G. Krishnan 2015
        # For FREE, we no longer assume constant volume
        # Radius, volume, and twist will be computed from the state
...
def _compute_all_dilatations(
    ...
    for k in range(lengths.shape[0]):
        ...
        # beta = beta_angle[k]
        # sqrt_alpha = np.sqrt(
        #     max(0.0, (1 + np.cos(alpha) * dilatation[k]))
        #     * max(0.0, (1 - np.cos(alpha) * dilatation[k]))
        # )
        # sqrt_beta = np.sqrt(
        #     max(0.0, (1 + np.cos(beta) * dilatation[k]))
        #     * max(0.0, (1 - np.cos(beta) * dilatation[k]))
        # )
        # lambda_2 = max(
        #     -(
        #         sgn_beta * sqrt_beta * np.cos(alpha)
        #         - sgn_alpha * sqrt_alpha * np.cos(beta)
        #     )
