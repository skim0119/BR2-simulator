...
def main():
    ...
    # Actuation Profile
    action = {"action1": 30 * psi2Nm2}

    # Prepare environment
    env = Environment(run_tag=args.tag)
    env.reset(
        rod_database_path="database/rod_library.json",
        assembly_config_path="assembly/free_twist.json",
    )

    # Simulation
    status = env.run(action=action, duration=5.0, check_nan=True, check_steady_state=True)
    print(status)
